// SMTP Email Service for automatic email sending via OVH or other SMTP providers
// This service sends emails through a backend API to protect SMTP credentials

export interface SMTPConfig {
  host: string;           // SMTP server (ex: ssl0.ovh.net)
  ...
  photographerEmail: string; // Email destinataire
...
class SMTPEmailService {
  private readonly CONFIG_KEY = 'smtp-email-config';
  private readonly API_ENDPOINT = '/api/send-smtp-email';
  ...
  // Save configuration
  saveConfig(config: SMTPConfig): boolean {
    try {
  ...
  // Send email via SMTP (through backend API)
  async sendEmail(payload: EmailPayload): Promise<{ success: boolean; error?: string; messageId?: string }> {
    try {
      ...
      if (!response.ok) {
        console.error('❌ SMTP email send failed:', result.error);
        return {
          success: false,
...
Cet email a été généré automatiquement par votre système de galerie photo.
  ...
  isConfigured(): boolean {
    const config = this.getConfig();
    return !!(config && config.photographerEmail && config.fromEmail && config.host && config.enableNotifications);
  }

  // Get default configuration for OVH
  getDefaultConfig(): SMTPConfig {
    return {
      host: 'ssl0.ovh.net',
      port: 587,
