...
export function FavoritesPage({ galleryId }: FavoritesPageProps) {
  ...
  useEffect(() => {
    const initMasonry = () => {
      if (!masonryRef.current || filteredFavoritePhotos.length === 0 || viewMode !== 'grid') return;
  ...
  const removeFromFavorites = async (photoId: string, e?: React.MouseEvent) => {
    ...
    const currentUser = userService.getCurrentSession();
    if (!currentUser) {
      toast.error('Vous devez vous identifier pour retirer des favoris');
      return;
    }
    
    // Check if this photo was favorited by the current user
    const userFavorite = favorites.find(
      fav => fav.photoId === photoId && fav.userId === currentUser.userId
    );
    
    if (!userFavorite) {
      toast.error('Vous ne pouvez retirer que vos propres favoris');
      return;
    }
