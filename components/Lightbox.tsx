...
import { X, ChevronLeft, ChevronRight, Heart, MessageSquare, Send, User } from "lucide-react";
import type { Comment, FavoritePhoto } from "../services/favoritesService";
...
interface LightboxProps {
  ...
  favoriteDetails?: FavoritePhoto[]; // Add favorite details for user info
...
export function Lightbox({
  ...
  // Get favorite info for current photo
  const currentPhotoFavorite = currentPhoto 
    ? favoriteDetails.find(f => f.photoId === currentPhoto.id)
    : null;
  ...
  return (
    <div className="lightbox-container animate-fadeIn">
                ...
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span className="font-medium text-gray-700">
                    {comment.userName ? (
                      ...
                      `Appareil: ${comment.deviceId.split('_')[1] || 'inconnu'}`
                    )}
                  </span>
      ...
      <div className="lightbox-controls">
        {/* User who added to favorites */}
        {isInSelection && currentPhotoFavorite?.userName && (
          <div className="flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-800 rounded-full">
            <User className="h-4 w-4" />
            <span className="text-sm font-medium">{currentPhotoFavorite.userName}</span>
          </div>
        )}
